...
const enhanceMarkdown = (html: string, isDarkMode: boolean): string => {
  const darkModeHighlight = isDarkMode ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.04)";

  const codeBackground = isDarkMode ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.06)";
  ...
  return html
    .replace(
      /<h2/g,
      `<h2 style="border-bottom: 1px solid ${borderColor}; padding-bottom: 0.5rem; margin-bottom: 1rem; margin-top: 2rem; font-size: 1.5rem; font-weight: 600;"`
    )
    .replace(
      /<h1/g,
      `<h1 style="border-bottom: 2px solid ${borderColor}; padding-bottom: 0.75rem; margin-bottom: 1.5rem; margin-top: 0.5rem; font-size: 1.75rem; font-weight: 700;"`
    )
    .replace(/<ul>/g, '<ul style="margin: 1rem 0; padding-left: 1.5rem;">')
    .replace(/<ol>/g, '<ol style="margin: 1rem 0; padding-left: 1.5rem;">')
    .replace(
      /<pre>/g,
      `<pre style="background: ${darkModeHighlight}; padding: 1rem; border-radius: 8px; overflow: auto; margin: 1rem 0;"`
    )
    .replace(
      /<code>/g,
      `<code style="font-family: monospace; font-size: 0.9rem; background: ${codeBackground}; padding: 0.2rem 0.4rem; border-radius: 4px;"`
    )
            ...
            <StyledDialogContent dividers>
              <Box
                ...
                dangerouslySetInnerHTML={{
                  __html: enhanceMarkdown(
                    DOMPurify.sanitize(marked.parse(readmeContent) as string),
                    isDarkMode
                  ),
                }}
              />
